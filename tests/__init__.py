...
if jsonpickle:
    _data_formats["json"] = {
        ...
        "writer": lambda x: json.dumps(
            json.loads(jsonpickle.encode(x)), indent=4, ensure_ascii=False
        ),
    }
...
def get_formatted_test_data(path):
    ...
    saved_test_data = get_test_data(
        path, _data_formats[data_format].get("read_mode", "r")
    )
