...
def test_simple_tiny_ranges_collapses_adjacent_pair():
    diff = _diff(
        added=(("vlan 1000", OrderedDict()), ("vlan 1001", OrderedDict())),
    )

    assert list(vlandb.process_vlandb({}, (), diff, tiny_ranges=True, exclude_added_blocks=False)) == [
        (True, "vlan 1000-1001", None)
    ]
...
def test_simple_preserves_affected_vlan_children():
    ...
    assert list(vlandb.simple_no_tiny_ranges({}, (), diff)) == [(True, "vlan 1000", children)]


def test_process_vlandb_can_exclude_new_vlan_blocks_from_plain_additions():
    children = OrderedDict({"name SERVERS": OrderedDict()})
    diff = _diff(added=(("vlan 1000", children),))

    assert list(
        vlandb.process_vlandb(
            {},
            (),
            diff,
            tiny_ranges=True,
