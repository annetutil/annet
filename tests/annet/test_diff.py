...
def str2diff(raw_diff: str) -> Diff:
    ...
    for raw_line in raw_diff.splitlines():
        ...
        if level == 0:
            ...
            raise Exception("such high level of nesting is not supported here")
    return res
...
def test_diff_sorting() -> None:
    with io.StringIO() as buf:
        diff = resort_diff(str2diff(diff_unsorted))
        pd = patching.make_pre(diff)
        for pre_as_diff in gen_pre_as_diff(pd, False, " ", False):
            buf.write(pre_as_diff)
        diff_sorted_actual = buf.getvalue()

    with io.StringIO() as buf:
        diff = str2diff(diff_sorted)
        pd = patching.make_pre(diff)
        for pre_as_diff in gen_pre_as_diff(pd, False, " ", False):
            buf.write(pre_as_diff)
