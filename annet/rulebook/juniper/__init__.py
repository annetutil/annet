...
def comment_processor(item: common.DiffItem) -> common.DiffItem:
...
def inactive_blocks(diff_foo: Callable[..., list[common.DiffItem]]) -> Callable[..., list[common.DiffItem]]:
    @functools.wraps(diff_foo)
    def wrapper(
        old: odict[str, Any], new: odict[str, Any], diff_pre: odict[str, Any], *args: Any, **kwargs: Any
...
# =====
def strip_toplevel_inactives(tree: odict[str, Any]) -> odict[str, Any]:
...
def activate_cmd(active_key: str, diff_pre: dict[Any, Any]) -> str:
    return cmd(active_key, diff_pre, "activate")


def deactivate_cmd(active_key: str, diff_pre: dict[Any, Any]) -> str:
...
def strip_inactive_removed(diff: list[common.DiffItem]) -> list[common.DiffItem]:
    for elem in diff:
